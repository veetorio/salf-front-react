...
function Usuarios() {
    const { data } = useQuery({
        queryKey: ["usuarios"],
        queryFn: async () => {
            return toast.promise(getUsuarios, {
    ...
    const [requestBody, setRequestBody] = useState({ ...defaultValues });
    const [postView, setPostView] = useState(false);
    const [isPost, setPost] = useState(false);
    const mutPost = useMutation({
        mutationKey: ['postUsuarios'],
        mutationFn: async (body) => postUsuarios(body)
    });
    const mutDelete = useMutation({
        mutationKey: ['deleteUsuarios'],
        mutationFn: async (id) => {
            toast.promise(deleteUsuarios(id), {
    ...
    const sendForm = async (data) => {
        if ((data?.password != data?.replyPassword) || (data?.password === "") || (data?.replyPassword === ""))
            ...
            if (isPost) {
                ...
                await toast.promise(updateUsuarios(requestBody.id, data || {
                    email: '',
                    name: '',
                    password: '',
                    role: ''
                    ...
                    <form onSubmit={handleSubmit(sendForm)} className="flex flex-col gap-4">
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <div className="flex flex-col">
                                ...
                                <Controller name="role" control={control} defaultValue="" render={({ field }) => <Dropdown {...field} value={field.value} onChange={(e) => { field.onChange(e.value); }} options={roles} optionValue="value" optionLabel="label" panelClassName="p-4" className="p-2" placeholder="selecione"/>}/>
                            </div>
                        </div>
    ...
    return <Base>
        ...
        <Form />
        <Table title="Lista de Usuários" rows={data ?? [{ name: '', email: '', id: 0, role: '' }]} filterInputs={inputsFilter} filterDrops={dropsFilter} isAct={true} options={[["ADMIN", "COORDINATOR", "MANAGER", "APPLICATOR"]]} deleteCallbacks={mutDelete.mutate} editCallbacks={mutUpdate.mutate}/>
    </Base>;
}
