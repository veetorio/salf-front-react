...
export interface ClassGroup {
    name: string;         // Nome da turma, ex: "5º Ano A"
    grade: string;        // Série da turma, ex: "5º ANO"
    schoolId: number;     // ID da escola associada
    turn: "MATUTINO" | "VESPERTINO" | "NOTURNO" | "INTEGRAL"; // Turno da turma
}
...
function Turmas() {
    ...
    const { register, handleSubmit, control , reset } = useForm<ClassGroup>()
    const PostTurmas = () => {
        ...
        const sendClass = (data: ClassGroup) => {
            if (post) postTurmas(data)
            else putTurmas(data,id);
        }
            ...
            <div className="p-4">
                ...
                <form action="" className="flex flex-col gap-4 mt-4" onSubmit={handleSubmit(sendClass)}>
                    ...
                    <div className="flex flex-col">
                        ...
                        <Controller
                            name="schoolId"   // melhor usar schoolId já que vai salvar o id
                            control={control}
                            render={({ field }) => (
                                <Select
                                    {...field}
                                    options={schools}
                                    value={schools?.find((s) => s.value === field.value) || null}
