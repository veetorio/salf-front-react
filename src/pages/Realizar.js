...
import { getRealizarAvaliacao, PassStage, postRealizarAvaliacao, } from "../api/api-realizar";
...
function Realizar() {
    ...
    const OptionSelect = ({ value, active, onToggle, }) => {
        return (<button className={`p-2 text-white border-none rounded-md hover:opacity-50 cursor-pointer 
    ...
    const calcParams = (percent, all) => Math.round((percent / 100) * all);
    // 🔹 Atualiza picked + respostas
    const handleToggle = (step, value) => {
        setPicked((prev) => {
    ...
    const [level, setlevel] = useState("NOT_EVALUATED");
    useEffect(() => {
        if (states.school)
            queries[1].refetch();
    }, [queries, states.school]);
    useEffect(() => {
        if (states.classSchool)
            queries[2].refetch();
    }, [queries, states.classSchool]);
    const optionSchool = queries[0].data?.map((e) => ({ value: e.id, label: e.name })) || [];
    const optionClass = queries[1].data?.map((e) => ({ value: e.id, label: e.name })) || [];
    const optionStudents = queries[2].data?.map((e) => ({ value: e.id, label: e.name })) || [];
    const optionAssements = queries[3].data?.map((e) => ({ value: e.id, label: e.name })) || [];
    const optionEvents = queries[4].data?.map((e) => ({ value: e.id, label: e.name })) || [];
    const stepsLogic = {
        "palavras": (size, all) => {
    ...
    const steps_element = useRef(null);
    // const erroNotify = () => alert("Error")
    const handlePassSteps = (step, index) => {
    ...
    const updateStage = (isCompleted, stage) => {
        const body = {
    ...
    const OptionsBox = (item) => {
        switch (item.nameStep.toLocaleLowerCase()) {
          ...
          {item.data.length !== 0 ? (item.data.map((e) => (<OptionSelect key={`${item.nameStep}-${e}`} value={e} step={item.nameStep} active={!!picked[item.nameStep]?.[e]} onToggle={() => handleToggle(item.nameStep, e)}/>))) : (<button onClick={() => steps_element.current?.nextCallback()} className="px-4 py-2 text-lg rounded-md border-none 
          ...
          {item.data.map(e => (<QuestionSelect key={e} step={item.nameStep} value={e} active={!!picked[item.nameStep]?.[e]} onEvent={handleToggle}/>))}
        </section>;
            default:
                return <section className="p-4 grid grid-cols-4 gap-2">
          {item.data.map((e) => (<OptionSelect key={`${item.nameStep}-${e}`} value={e} step={item.nameStep} active={!!picked[item.nameStep]?.[e]} onToggle={() => handleToggle(item.nameStep, e)}/>))}
        </section>;
        }
    };
    ...
    const QuestionSelect = ({ value, active, onEvent, step }) => {
        return (<div className="flex flex-col gap-3 bg-gray-1 mt-4 p-2 rounded justify-end">
        <span>{value}</span>
        <span className="flex gap-2 items-baseline">
          Está correta ?
          <Checkbox checked={active} onChange={() => onEvent(step, value)}/>
        </span>
      </div>);
    };
    ...
    return (<>
      ...
      <Base>
        <BoxDefault title="Realizar Avaliação de Leitura" subtitle="Avalie a fluência de leitura em três etapas"></BoxDefault>
        ...
        {!mut.isSuccess && (<section title="Selecione a Avaliação" className="mt-4 w-full px-2 py-9 rounded-xl border border-solid border-gray-2 shadow-md">
            ...
            <form className="grid grid-cols-3 gap-x-2 gap-y-4 mt-4" onSubmit={async (e) => {
                e.preventDefault();
              ...
              <div>
                ...
                <Select className="mt-2" placeholder="selecione a escola" options={optionSchool} isLoading={queries[0].isLoading} onChange={(inp) => setStates((e) => ({ ...e, school: inp?.value ?? 0 }))}/>
              </div>

              <div>
                <label htmlFor="" className="c-blue-950 font-bold">Turmas</label>
                <Select className="mt-2" placeholder="selecione a turma" options={optionClass} isLoading={queries[1].isLoading} onChange={(inp) => setStates((e) => ({ ...e, classSchool: inp?.value ?? 0 }))}/>
              </div>
              <div>
                <label htmlFor="" className="c-blue-950 font-bold">Estudantes</label>
                <Select className="mt-2" placeholder="selecione o estudante" options={optionStudents} isLoading={queries[2].isLoading} onChange={(inp) => setStates((e) => ({ ...e, student: inp?.value ?? 0 }))}/>
              </div>
              <div>
                <label htmlFor="" className="c-blue-950 font-bold">Avaliações</label>
                <Select className="mt-2" placeholder="selecione a avaliação" isLoading={queries[3].isLoading} options={optionAssements} onChange={(inp) => setStates((e) => ({ ...e, assements: inp?.value ?? 0 }))}/>
              </div>
              <div>
                <label htmlFor="" className="c-blue-950 font-bold">Eventos</label>
                <Select className="mt-2" placeholder="selecione o evento" options={optionEvents} isLoading={queries[4].isLoading} onChange={(inp) => setStates((e) => ({ ...e, events: inp?.value ?? 0 }))}/>
              </div>
        ...
        <main className="h-full w-full">
          {mut.isSuccess && (<Stepper linear ref={steps_element} className="p-4 mt-8 shadow-2xl rounded-xl border border-solid border-gray-2" headerPosition="left">
              {steps.map((item, index) => (<StepperPanel key={item.nameStep} header={item.nameStep}>
                  <main className="h-full w-full mt-8">
                    <header className="p-4 flex flex-col gap-4">
                      <div className="w-full flex justify-between">
                        <h1>Etapa - {item.nameStep}</h1>
                        <div className="flex gap-4">
                          {item.nameStep === "palavras" && <button onClick={encerrar} className="p-2 border-none rounded-md bg-red-6 hover:bg-red text-white">
                              encerrar
                            </button>}
                    ...
                    <footer className="flex justify-end">
                      <button className={`${isDisabled ? "bg-blue" : "bg-gray"} hover:opacity-50 text-white border-none px-4 py-5 rounded-xl`} onClick={() => {
                    if (isDisabled) {
                        handlePassSteps(item.nameStep, index);
                    }
                }}>
                        proxima etapa
                      </button>
              ...
              <StepperPanel header="Resultado">
                ...
                setPerfomance({ ...perf });
                    ...
                    <ButtonSecundary onClick={location.reload}>
