...
import { deleteEventos, getEvento, getEventos, postEventos, putEventos } from "../api/api-eventos";
...
function Avaliacoes() {
    ...
    const Avaliacoes = () => {
        ...
        const QuestionForm = ({ text, options, id }) => {
            const autoRemove = (index) => {
                setAssement(prev => {
                    // se não existir a questão, só retorna o estado anterior
                    if (!prev.questions[id])
                        return prev;
                    const updatedQuestions = prev.questions.map((q, i) => i === id
            ...
            const internInputRef = useRef(null);
            const removeQuestion = () => {
                const filterQuest = assement.questions.filter((_, i) => i != id);
                assement.questions = filterQuest;
                setAssement({ ...assement });
            };
            return (<div className="bg-gray-2 p-4 mt-3">
                    <div className="flex justify-between">
                        <div>
                            <label><h4>Enunciado da questão</h4></label>
                            <InputText className="w-full mt-2 p-2" placeholder="coloque o enunciado" defaultValue={text} onChange={(event) => {
                    ...
                    <div className="flex flex-col mt-2">
                        <div className="flex flex-col gap-3">
                            ...
                            {options.map((e, index) => (<div className="p-inputgroup flex-1">
                        ...
                        <div className="flex mt-2 items-end justify-between">
                            <ButtonPrime type="button" onClick={() => {
        ...
        const inputPseud = useRef(null);
        const inputPhrases = useRef(null);
        const onPush = (field, fieldRef, max) => {
            console.log(assement);
            if (fieldRef?.current?.value.length > 0 && fieldRef?.current?.value.length <= max) {
                const chips = (fieldRef?.current?.value).split(/[.,]/g);
                for (const item of chips)
                    assement[field].push(item.trim());
                setAssement({ ...assement });
            }
            else {
        ...
        const onPushPhrase = (field, fieldRef, max) => {
            if (fieldRef?.current?.value.length > 0 && fieldRef?.current?.value.length <= max) {
                const chips = (fieldRef?.current?.value).split(/[.]/).map(e => ({ text: e }));
                for (const item of chips)
                    assement[field].push(item);
                setAssement({ ...assement });
            }
            else {
                toast.error("adicione um item nesse campo");
    ...
    const Eventos = () => {
        ...
        const mutPost = useMutation({
            mutationFn: async (data) => sendSubmitEvent(data)
        });
        const mutDelete = useMutation({
            mutationFn: async (id) => deleteEventFn(id)
        });
        const mutPut = useMutation({
            mutationFn: async (body) => putEventos({ ...body }, id)
        });
        ...
        const sendSubmitEvent = (data) => {
            postEventos(data);
        };
        const deleteEventFn = (id) => {
            deleteEventos(id);
        };
        const UpdateEvent = () => {
