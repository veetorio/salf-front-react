...
function Dashboard() {
    ...
    const { handleSubmit, control } = useForm<DashboardData>()
    const [cards, setCards] = useState<DashboardAnalytics>({
        averagePpm: 0,
    ...
    const [lineProgress, setLineProgress] = useState<Evolution>(
        {
            distribution: [],
            eventId: 0,
            eventName: ""
    ...
    const sendForm = async (data: DashboardData) => {
    ...
    const Card = (props: { isPercent?: boolean, value: number, color: "bg-purple" | "bg-blue" | "bg-green" | "bg-yellow", text: string, icon: ReactNode }) => {
    ...
    return <Base>
        <BoxDefault
            ...
            subtitle="Visualize e analise os dados das avaliações de leitura">
        </BoxDefault>
        <section>
