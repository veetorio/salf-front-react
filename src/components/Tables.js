...
function Table(props) {
    const buttons = (row) => {
        return <div className='gap-2'>
            <button className='c-orange-6 border-none text-xl bg-transparent hover:c-orange-9 p-2' onClick={() => {
                props.editCallbacks(Number(row.id));
            }}>
                <MdEdit />
            </button>
            <button className='c-red-6 border-none text-xl bg-transparent hover:c-red-9 p-2' onClick={() => {
    ...
    return <div className="h-fit w-full mt-4 border-solid border-1 border-gray-2 shadow-md p-4 rounded-xl">
        <header className='w-full flex flex flex-col gap-4'>
            ...
            <div className='flex flex-wrap w-full bg-gray-1 p-2 flex gap-2'>
                {inputsFilter.map(item => <InputText placeholder={`pesquise por ${translation[item]}`} className='p-2' onChange={(e) => setChange(item, e.target.value)}/>)}{dropsFilter.map((item, i) => <Dropdown options={(props.options ?? [[]])[i]} value={(filter[item] || { value: '' }).value} className='p-2 w-fit' panelClassName='px-2 py-4 border-none' placeholder={`pesquise por ${translation[item]}`} onChange={(e) => setChange(item, e.target.value)}/>)}
                <Button onClick={clear}>Limpar filtros</Button>
            </div>
        </header>
        <DataTable paginator rows={10} value={props.rows ?? []} filters={filter} onFilter={(e) => setFilter(e)} autoSave='' emptyMessage="nenhum item foi encontrado" paginatorClassName='justify-end' className='p-4'>
            {cols.map(e => <Column field={e} header={translation[e]} body={bodyTemplate} headerClassName='p-2 text-xs' bodyClassName={'p-2 text-sm'}/>)}{props.isAct && <Column field='act' header='ações' className='p-2' body={buttons}/>}
        </DataTable>
    </div>;
}
